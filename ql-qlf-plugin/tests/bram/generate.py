...
with open(f"{TEST_DIR}Makefile", "w") as mf:
    ...
    mf.write("\n")
    mf.write("clean:\n")
    mf.write(f"\trm -f Makefile")
    for t in TESTS:
        mf.write(f" t_{t.name}.v t_{t.name}.ys t_{t.name}_count.old t_{t.name}_mapped_old.il t_{t.name}_count.new t_{t.name}_mapped_new.il")
    mf.write("\n")
    mf.write(".PHONY: all clean\n")

    for t in TESTS:
        with open(f"{TEST_DIR}t_{t.name}.v", "w") as tf:
            tf.write(t.src)
        with open(f"{TEST_DIR}t_{t.name}.ys", "w") as sf:
            sf.write(f"read_verilog t_{t.name}.v\n")
            sf.write(f"hierarchy -top top\n")
            sf.write(f"design -save orig\n")
            sf.write(f"scratchpad -set ql.lib_path \"{LIB_PATH}\"\n")
            sf.write(f"synth_quicklogic -top top -family qlf_k6n10f\n")
            sf.write(f"stat; tee -o t_{t.name}_count.old select -count t:TDP36K\n")
            sf.write(f"write_rtlil t_{t.name}_mapped_old.il\n")
            sf.write(f"design -load orig\n")
            sf.write(f"script ../ql_flow.ys\n")
            sf.write(f"stat; tee -o t_{t.name}_count.new select -count t:TDP36K\n")
            sf.write(f"write_rtlil t_{t.name}_mapped_new.il\n")
        mf.write(f"\n{t.name}:\n")
        mf.write(f"""\t@echo "t_{t.name}:"\n""")
        mf.write(f"\t@yosys -m {LIB_PATH}build/ql-qlf.so -qL t_{t.name}.log t_{t.name}.ys\n")
        mf.write(f"""\t@echo "Old mem count: `cat t_{t.name}_count.old`"\n""")
        mf.write(f"""\t@echo "New mem count: `cat t_{t.name}_count.new`"\n""")
        mf.write(f".PHONY: {t.name}\n")
